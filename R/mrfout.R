...
print.mrfout <- function(x, ...) {
  M <- x$mrfi@Rmat
  pos <- split(M, rep(seq_len(nrow(M)), times = 2))
...
summary.mrfout <- function(object, ...) {
  ...
  mrfi <- object$mrfi
  C <- nrow(theta) - 1
  potentials <- lapply(seq_len(nrow(object$mrfi@Rmat)),
                       function(x) {
                         smr_array(object$theta[, , x, drop = FALSE], family)})
  contributions <- sapply(seq_len(nrow(object$mrfi@Rmat)),
                          function(x) {
                            sum(abs(cohist(object$Z, object$mrfi)[, , x] *
                                    object$theta[, , x]))
                          }
  )
  contributions <- unlist(contributions)
  contributions <- contributions / max(contributions)
  codes <- cut(contributions,
  ...
  if (family == "onepar") {
    equals <- apply(cohist(object$Z, mrfi),
                    function(x) sum(diag(x)),
                    MARGIN = 3)
    diffs <- apply(cohist(object$Z, mrfi),
                   function(x) sum(x) - sum(diag(x)),
                   MARGIN = 3)
    ...
    if (family == "oneeach") {
      ...
      cat("Position|",
          sprintf("%-6s",
                  paste0(rep(0:C, C + 1), ",", rep(0:C, each = C + 1))[-1]),
                  " Rel. Contribution")
      cat("\n")
      ...
      cat("Position|",
          sprintf("%-6s",
                  vec_description(mrfi, family, C)$interaction[1:((C + 2) * (C + 1) / 2 - 1)]),
                  " Rel. Contribution")
      cat("\n")
    }
    for (i in seq_along(pos)) {
...
plot.mrfout <- function(x, ...) {
  vecs <- lapply(seq_len(nrow(x$mrfi@Rmat)),
                 function(i) {
                   smr_array(x$theta[, , i, drop = FALSE], family = x$family)})
